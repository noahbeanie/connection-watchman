...
export function StatusBadge({ live, meta }: { live: Live | null; meta: Meta | null }) {
  ...
  return (
    <Badge
      ...
      className="h-auto shrink-0 cursor-default gap-1.5 py-1 pl-2.5 pr-3 text-sm font-semibold sm:gap-3 sm:py-3 sm:pl-5 sm:pr-7 sm:text-3xl"
      style={{
    ...
    >
      <span className="relative flex size-2.5 sm:size-4">
        {state === "up" && (
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full opacity-60" style={{ background: c }} />
        )}
        <span className="relative inline-flex size-2.5 rounded-full sm:size-4" style={{ background: c }} />
      </span>
      {text}
    </Badge>
  )
